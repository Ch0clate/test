...
public class AccoSearchProcessor implements AccoConstants {
	...
	public static void setSupplierRequestElem(JSONObject reqJson,Element reqElem) throws Exception {
		...
		createSuppReqHdrElem(XMLUtils.getFirstElementAtXPath(reqElem, "./acco:RequestHeader"),sessionID,transactionID,userID);
		UserContext usrCtx = UserContext.getUserContextForSession(reqHdrJson);
		List<ProductSupplier> prodSuppliers = usrCtx.getSuppliersForProduct(PROD_CATEG_ACCO,prodCategSubtype!=null?prodCategSubtype.toString():DEF_PRODSUBTYPE);
	...
	public static void createSuppReqHdrElem(Element reqHdrElem, String sessionId, String transactId, String userId) {
	...
	public static void setSuppReqOTAElem(Document ownerDoc,Element reqOTAElem,JSONObject reqParamJson,JSONObject reqHdrJSON) {
		...
		//TODO:country code or market should be added.It wont come from header
	...
	public static JSONObject getRoomStayJSON(Element roomStayElem) {
		...
		roomStayJson.put(JSON_PROP_PAYATHOTEL,!Boolean.valueOf(XMLUtils.getValueAtXPath(roomStayElem, "./ota:RatePlans/ota:RatePlan/@PrepaidIndicator")));
	...
	@Deprecated
	public static JSONObject getRoomCategoryJson(Element roomCategoryElem) {

		JSONObject roomCategoryJson = new JSONObject();

		roomCategoryJson.put("name", XMLUtils.getValueAtXPath(roomCategoryElem, "./acco:RoomCategoryName"));
		roomCategoryJson.put("code", XMLUtils.getValueAtXPath(roomCategoryElem, "./acco:RoomCategoryID"));

		return roomCategoryJson;
	...
	public static String process(JSONObject reqJson) {
		try{
			...
			//CompanyOffers.getCompanyOffers(reqJson, resJson,resClientComJson,BRMS2SIRoomMap);
	...
	public static void calculatePrices(JSONObject reqJson, JSONObject resJson, JSONObject clientCommResJson,JSONObject suppCommResJson, Map<Integer,String> SI2BRMSRoomMap,boolean retainSuppFaresAndComm) {
		...
		String suppId=DEF_SUPPID;
		boolean isMrkpAdded,isRecvblsAdded;
		BigDecimal totalPrice,newTotalPrice,totalTaxPrice;
		...
		for(int i=0;i<resAccoInfoJsonArr.length();i++) {
			...
			for(int j=0;j<resRoomJsonArr.length();j++) {
				 ...
				 */
				//---------------Initialize,apply ROE,retain fares--------------------
				resRoomJson = resRoomJsonArr.getJSONObject(j);
				String commercialCurrency = resRoomJson.getJSONObject(JSON_PROP_ROOMPRICE).getString(JSON_PROP_CCYCODE);
				...
				applyRoe(newTotalPriceJson, clientCcyCode, clientMarket);
				applyRoe(newTotalTaxJson, clientCcyCode, clientMarket);
				if(newTotalTaxJson.has(JSON_PROP_TAXBRKPARR)) {
					...
					for(Object taxJson:newTaxJsonArr) {
						applyRoe((JSONObject) taxJson, clientCcyCode, clientMarket);
					}
				}
				for(Object nghtPriceJson:newNghtlyPriceJsonArr) {
					applyRoe((JSONObject) nghtPriceJson, clientCcyCode, clientMarket);
					applyRoe(((JSONObject) nghtPriceJson).getJSONObject(JSON_PROP_TOTALTAX), clientCcyCode, clientMarket);
				}
				...
				if(retainSuppFaresAndComm) {
					resRoomJson.put(JSON_PROP_SUPPROOMPRICE, newTotalPriceJson);
					resRoomJson.put(JSON_PROP_SUPPNIGHTLYPRICEARR, newNghtlyPriceJsonArr);
				}
				//---------------------------------------------------------------------------
				
				/**
				 * SI2BRMS map has been populated during supplie commercials.
				...
				if(retainSuppFaresAndComm) {
					JSONArray clntEntityCommJsonArray = getClntEntityCommJsonArray(entityCommJsonArr, ccommToTypeMap,reqJson,commercialCurrency);
					JSONArray suppCommJsonArray = getSuppCommJsonArray(ccommRoomJson.optJSONArray(JSON_PROP_COMMDETAILS), scommToTypeMap,commercialCurrency);
				...
				for (int l = (entityCommJsonArr.length() - 1); l >= 0 && !(isMrkpAdded && isRecvblsAdded); l--) {
					...
					if (markupCalcJson != null && !isMrkpAdded) {
						BigDecimal markupCalcRoe = RedisRoEData.getRateOfExchange(markupCalcJson.optString(JSON_PROP_COMMCCY,""), clientCcyCode, clientMarket);
						...
						for (int x=0; x < additionalCommsJsonArr.length(); x++) {
							...
							if (COMM_TYPE_RECEIVABLE.equals(ccommToTypeMap.get(additionalCommsJson.optString(JSON_PROP_COMMNAME)))) {//is the additionalCommName receivable?
								...
								if(additionalCommAmt!=null) {
									additionalCommAmt = additionalCommAmt.multiply(RedisRoEData.getRateOfExchange(additionalCommCcy, clientCcyCode, clientMarket));
									newTotalPrice = newTotalPrice.add(additionalCommAmt);
								}
								//TODO:add receivables
								/*receivableJson = new JSONObject();
								receivableJson.put(JSON_PROP_AMOUNT, additionalCommAmt);
								receivableJson.put(JSON_PROP_CCYCODE, clientCcyCode);
								receivableJson.put(JSON_PROP_CODE, additionalCommsJson.optString(JSON_PROP_COMMNAME));
								receivableJsonArr.put(receivableJson);
	...
	public static Map<String, String> getClientCommercialsAndTheirType(JSONObject clientCommResJson) {
		...
		for (int i=0; i < ccommBRIJsonArr.length(); i++) {
			if ((entDetaiJsonArray = ccommBRIJsonArr.getJSONObject(i).getJSONArray(JSON_PROP_ENTITYDETAILS)) == null) {
				continue;
			}
			for(int j=0;j<entDetaiJsonArray.length();j++){
				if((commHeadJsonArr=entDetaiJsonArray.getJSONObject(j).getJSONArray(JSON_PROP_COMMHEAD)) == null) {
	...
	private static JSONArray getSuppCommJsonArray(JSONArray suppCommDtlsJsonArr,Map<String,String> scommTypeMap,String commercialCurrency) {

		JSONArray suppCommJsonArr = new JSONArray();

		if(suppCommDtlsJsonArr == null)
			// TODO:log message that no supplier comm found
			return suppCommJsonArr;

		suppCommDtlsJsonArr.forEach(suppCommDtlsJson -> addCommercialJson(suppCommJsonArr, (JSONObject) suppCommDtlsJson, scommTypeMap,commercialCurrency));

		return suppCommJsonArr;
	}

	private static JSONArray getClntEntityCommJsonArray(JSONArray entityCommJsonArr,Map<String,String> ccommTypeMap,JSONObject reqJson,String commercialCurrency) {

		JSONArray clntEntityCommJsonArr = new JSONArray();

		if (entityCommJsonArr == null)
			// TODO:log message that no client entity comm found
			return clntEntityCommJsonArr;

		entityCommJsonArr.forEach(clntentityJson -> addClientEntityJson(clntEntityCommJsonArr,(JSONObject) clntentityJson, ccommTypeMap,reqJson,commercialCurrency));

		return clntEntityCommJsonArr;
	}

	private static void addClientEntityJson(JSONArray clntEntityJsonArr, JSONObject clntentityJson,
			Map<String,String> ccommTypeMap,JSONObject reqJson,String commercialCurrency) {
		...
		entityJson.put(JSON_PROP_ENTITYNAME, clntentityJson.getString(JSON_PROP_ENTITYNAME));
		  /*entityJson.put(JSON_PROP_CLIENTID, "");
		  entityJson.put(JSON_PROP_COMMENTITYID,"");
		  entityJson.put(JSON_PROP_COMMENTITYTYPE,"");
		  entityJson.put(JSON_PROP_PARENTCLIENTID,"");
		  entityJson.put(JSON_PROP_ENTITYNAME, clntentityJson.getString(JSON_PROP_ENTITYNAME));*/
		  entityJson.put(JSON_PROP_CLIENTCOMM, ccommJsonArr);
		// insert markup
		addCommercialJson(ccommJsonArr, clntentityJson.optJSONObject(JSON_PROP_MARKUPCOMDTLS), ccommTypeMap,commercialCurrency);
		...
		if(addCommDtlsJsonArr!=null) {
			addCommDtlsJsonArr.forEach(addCommDtlsJson -> addCommercialJson(ccommJsonArr, (JSONObject) addCommDtlsJson, ccommTypeMap,commercialCurrency));
		}

		clntEntityJsonArr.put(entityJson);
	}

	private static void addCommercialJson(JSONArray commJsonArr,JSONObject commJson,Map<String,String> commTypeMap,String commercialCurrency) {

		if(commJson == null)
			return;
		JSONObject newCommJson = new JSONObject();

		newCommJson.put(JSON_PROP_COMMCCY, commJson.optString(JSON_PROP_COMMCCY, commercialCurrency));
	...
	private static void applyRoe(JSONObject priceJson,String toCcy, String market) {
		if(priceJson == null)
			return;
		if(priceJson.has(JSON_PROP_AMOUNT)) {
			BigDecimal currAmt = priceJson.getBigDecimal(JSON_PROP_AMOUNT);
			priceJson.put(JSON_PROP_AMOUNT, currAmt.multiply(RedisRoEData.getRateOfExchange(priceJson.optString(JSON_PROP_CCYCODE, ""), toCcy, market)));
