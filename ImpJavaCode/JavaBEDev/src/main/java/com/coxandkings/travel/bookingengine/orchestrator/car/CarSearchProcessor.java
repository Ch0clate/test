...
public class CarSearchProcessor implements CarConstants {
	...
	public static void getSupplierResponseVehicleAvailJSON(Element resBodyElem, JSONArray vehicleAvailJsonArr,
		...
		for (Element vehVendorElem : vehVendorElems) {
			JSONObject vehicleAvailJson = getVehicleAvailJSON(vehVendorElem);
			// TODO : SupplierID namespace is not finalized by SI
			String suppId = XMLUtils.getValueAtXPath((Element) resBodyElem.getParentNode().getParentNode(),
					"./car:SupplierID");
			if (generateBookRefIdx) {
				vehicleAvailJson.put(JSON_PROP_BOOKREFIDX, bookRefIdx);
			}
			// vehicleAvailJson.put("VehicleRentalInfo", vehRentalJson);
			vehicleAvailJson.put(JSON_PROP_SUPPREF, suppId);
			...
			vehicleAvailJsonArr.put(vehicleAvailJson);
		}
	}

	private static JSONObject getVehicleAvailJSON(Element vehVendorElem) throws Exception {

		JSONObject vehVehicleJson = new JSONObject();

		Element vehicleElem = XMLUtils.getFirstElementAtXPath(vehVendorElem,
				"./ota:VehAvails/ota:VehAvail/ota:VehAvailCore/ota:Vehicle");
		vehVehicleJson.put(JSON_PROP_VEHICLEINFO, getVehicleInfo(vehicleElem));
		
		Element vehAvailElem = XMLUtils.getFirstElementAtXPath(vehVendorElem, "./ota:VehAvails/ota:VehAvail");
		...
		vehVehicleJson.put("status", XMLUtils.getValueAtXPath(vehAvailCoreElem, "./@Status"));
		...
		Element rateDistElems[] = XMLUtils.getElementsAtXPath(vehVendorElem,
				"./ota:VehAvails/ota:VehAvail/ota:VehAvailCore/ota:RentalRate/ota:RateDistance");
		vehVehicleJson.put(JSON_PROP_RATEDISTANCE, getRateDistanceJSON(rateDistElems));

		Element rateQualifierElem = XMLUtils.getFirstElementAtXPath(vehVendorElem,
				"./ota:VehAvails/ota:VehAvail/ota:VehAvailCore/ota:RentalRate/ota:RateQualifier");
		vehVehicleJson.put(JSON_PROP_RATEQUALIFIER, getRateQualifierJSON(rateQualifierElem));

		Element referenceElem = XMLUtils.getFirstElementAtXPath(vehVendorElem,
				"./ota:VehAvails/ota:VehAvail/ota:VehAvailCore/ota:Reference");
		vehVehicleJson.put(JSON_PROP_REFERENCE, getReferenceJSON(referenceElem));
		vehVehicleJson.put(JSON_PROP_ISPREPAID,
				XMLUtils.getValueAtXPath(referenceElem, "./ota:TPA_Extensions/car:VehAvailDetails/@IsPrepaid"));
	...
	protected static JSONObject getLocationDetailsJSON(Element locationDetailsElem) {
		...
		/*
		 * String days[] = {"Mon","Tue","Weds","Thur","Fri","Sat","Sun"}; int i=0;
		 * for(Element operatingTimeElem :
		 * XMLUtils.getElementsAtXPath(locationDetailsElem,
		 ...
		 * operatingtimeJson.put("End", XMLUtils.getValueAtXPath(operatingTimeElem,
		 * "./@End")); operatingtimeJson.put("Start",
		 * XMLUtils.getValueAtXPath(operatingTimeElem, "./@Start")); //TODO: To Better
		 * Handle the Weekdays name for Supplier Hertz operatingtimeJson.put(days[i],
		 * XMLUtils.getValueAtXPath(operatingTimeElem, String.format("./@%s",
		 * days[i]))); operatingTimesArr.put(operatingtimeJson); i++; }
		 */
		...
		for (Element operatingTimeElem : XMLUtils.getElementsAtXPath(locationDetailsElem,
			...
			operatingtimeJson.put("day", getDay(operatingTimeElem));
	...
	protected static String getDay(Element elem) {
	...
	protected static JSONObject getVehicleInfo(Element vehicleElem) {
		...
		// VehType
		vehicleJson.put("vehicleCategory", XMLUtils.getValueAtXPath(vehicleElem, "./ota:VehType/@VehicleCategory"));
		vehicleJson.put("doorType", XMLUtils.getValueAtXPath(vehicleElem, "./ota:VehType/@DoorType"));
		vehicleJson.put("vehicleClassSize", XMLUtils.getValueAtXPath(vehicleElem, "./ota:VehType/@Size"));
		// VehMakeModel
	...
	protected static JSONObject getPricingInfoJSON(Element vehAvailCoreElem, Element vehAvailInfoElem) {
		...
		String cCyCode = "";
		Element vehChargeElems[] = XMLUtils.getElementsAtXPath(rentalRateElem,
				"./ota:VehicleCharges/ota:VehicleCharge");
	...
	public static String process(JSONObject reqJson) throws Exception {
		try {
			OperationConfig opConfig = CarConfig.getOperationConfig("search");
			...
			JSONObject resSupplierJson = SupplierCommercials.getSupplierCommercials(reqJson, resJson, suppResToBRIIndex);
	...
	private static void populateRequestBody(Element reqElem, Document ownerDoc, JSONObject carRentalReq)
			throws Exception {
		...
		if ((Utils.isStringNotNullAndNotEmpty(carRentalReq.optString(JSON_PROP_BOOKINGDURATION)))
				&& (carRentalReq.optString(JSON_PROP_BOOKINGUNIT) != null
						&& !carRentalReq.optString(JSON_PROP_BOOKINGUNIT).equals(""))) {
			Element Elem = CarSearchProcessor.getBookingDetailsElement(ownerDoc, carRentalReq);
	...
	protected static Element getVehRentalCoreElement(Document ownerDoc, JSONObject carRentalReq) throws Exception {
		...
		if (Utils.isStringNotNullAndNotEmpty(carRentalReq.optString(JSON_PROP_TRIPTYPE))) {
			...
			String tripType = carRentalReq.getString(JSON_PROP_TRIPTYPE);
			vehRentalElem.setAttribute("OneWayIndicator", tripType.equals("OneWay") ? "true" : "false");
		}
	...
	public static void calculatePricesV2(JSONObject reqJson, JSONObject resJson, JSONObject suppCommResJson,
		...
		JSONArray resCarRentalInfoArr = resJson.getJSONObject(JSON_PROP_RESBODY).getJSONArray(JSON_PROP_CARRENTALARR);
				...
				for (int k = 0; clientEntityCommJsonArr != null && k < clientEntityCommJsonArr.length(); k++) {
					...
					for (int p = 0; p < additionalCommsJsonArr.length(); p++) {
							...
							if (COMM_TYPE_RECEIVABLE.equals(additionalCommType)) {
								...
								totalFareCompsGroup.add(mRcvsPriceCompQualifier.concat(additionalCommName).concat("@").concat(JSON_PROP_CODE), clientCcyCode, additionalCommAmt);
							}
						}
					}
    				...
    				JSONObject baseFareJson = new JSONObject();
    				baseFareJson.put(JSON_PROP_AMOUNT, baseFareAmt.multiply(RedisRoEData.getRateOfExchange(markupCalcJson.optString(JSON_PROP_COMMCCY), clientCcyCode, clientMarket)));
