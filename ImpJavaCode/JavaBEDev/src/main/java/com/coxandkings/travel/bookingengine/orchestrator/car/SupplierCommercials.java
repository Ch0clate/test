...
public class SupplierCommercials implements CarConstants {
    ...
    public static JSONObject getSupplierCommercials(JSONObject req, JSONObject res, Map<String,Integer> suppResToBRIIndex) throws Exception{
...
//      JSONArray vehicleAvailJsonArr = resBody.getJSONArray(JSON_PROP_VEHICLEAVAIL);
        JSONArray vehicleAvailJsonArr = new JSONArray();
        JSONArray vehicleDetailsJsonArr = null;
        JSONArray multiResArr = resBody.getJSONArray(JSON_PROP_CARRENTALARR);
        String prevSuppId;
...
//    	vehDetails.put("modeOfPayment", vehicleAvailJson.getString("isPrepaid"));
		...
		/*	String cityName = vehicleAvailJson.getString(JSON_PROP_CITY);
	    	Map<String,String> cityInfo = RedisCityData.getCityInfo(cityName);
	    	...
	    	vehDetails.put(JSON_PROP_TOCITY, cityInfo.getOrDefault("city", ""));*/
...
//    	psgDetails.put("passengerType","ADT");
    	...
    	psgDetails.put(JSON_PROP_FAREBREAKUP, fareBreakUp);
    ...
    private static JSONObject getFareBreakUpJson(JSONObject vehicleAvailJson) {
    	...
    	if(pricedEquipsArr!=null && pricedEquipsArr.length()!=0) {
    		for(int i=0;i<pricedEquipsArr.length();i++){
    			...
    			if(pricedEquipsJson.optString(JSON_PROP_ISINCLDINBASE).equals("true")) {
    				baseTotal = baseTotal.subtract(pricedEquipsJson.getBigDecimal(JSON_PROP_AMOUNT));
...
//    		totalFareJson.getJSONObject(JSON_PROP_BASEFARE).put(JSON_PROP_AMOUNT, baseTotal);
    	...
    	if((taxJsonArr==null || taxJsonArr.length()==0) && (feesJsonArr==null || taxJsonArr.length()==0)) {
    		...
    		if(taxJsonArr!=null)
	    		for(int i=0;i<taxJsonArr.length();i++) {
	    			String code;
    		...
    		if(feesJsonArr!=null)
	    		for (int j=0; j < feesJsonArr.length(); j++) {
	                ...
	                if(!feeJson.optString(JSON_PROP_ISINCLDINBASE).equals("true")) {
		                feeDetailJson.put(JSON_PROP_TAXNAME, (code = feeJson.optString(JSON_PROP_FEECODE)).isEmpty()? "YQ": code);
