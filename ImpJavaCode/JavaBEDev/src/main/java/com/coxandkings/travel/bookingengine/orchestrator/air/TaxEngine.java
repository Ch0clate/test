...
public class TaxEngine implements AirConstants {
	...
	public static void getCompanyTaxes(JSONObject reqJson, JSONObject resJson) {
        ...
        createSupplierAndTravelDetails(usrCtx, pricedItinJsonArr, suppHdrJsonMap, suppTaxReqJson);
	...
	private static void createSupplierAndTravelDetails(UserContext usrCtx, JSONArray pricedItinJsonArr, Map<String,JSONObject> suppHdrJsonMap,Map<String, Map<String, JSONObject>> suppTaxReqJson) {
		OrgHierarchy orgHierarchy = usrCtx.getOrganizationHierarchy();
        for (int i=0; i < pricedItinJsonArr.length(); i++) {
        	...
        	if (travelDtlsJson == null) {
        		...
        		Map<String,Object> airportInfo = RedisAirportDataV2.getAirportInfo(destAirport);
        		if ( airportInfo != null) {
	...
	private static void addTaxesToResponseItinerariesTotal(JSONObject reqJson, JSONObject resJson, JSONObject taxEngResJson) {
		...
		for (int i=0; i < pricedItinsJsonArr.length(); i++) {
			...
			Map<String,Object> airportData = RedisAirportDataV2.getAirportInfo(destAirport);
			Map<String, JSONArray> travelDtlsMap = taxBySuppDestMap.get(suppID);
			String travelDtlsMapKey = String.format("%s|%s|%s", airportData.getOrDefault(RedisAirportDataV2.AIRPORT_CITY, ""), airportData.getOrDefault(RedisAirportDataV2.AIRPORT_STATE, ""), airportData.getOrDefault(RedisAirportDataV2.AIRPORT_COUNTRY, ""));
