...
public class SupplierCommercials implements BusConstants{
	...
	public static JSONObject getSupplierCommercials(JSONObject reqJson, JSONObject resJson,Map<String,Integer> BRMS2SIBusMap) {
	        ...
	        for (int i=0; i < availArr.length(); i++) 
	        {
		        	...
		        	if(serviceJsonArr!=null)
		        	{
			        	for(int j=0;j<serviceJsonArr.length();j++)
			        	{
			        		...
			        		JSONArray faresArr = serviceJsonArr.getJSONObject(j).getJSONArray("fares");
...
//		serviceDetails.putOnce("operatorName", "null");
		...
		//DATA FROM RADIS
//		String source = reqBody.getString(JSON_PROP_SOURCEID);
//    	Map<String,String> cityInfo = RedisCityData.getCityInfo(source);
...
//
//    	String destination = reqBody.getString(JSON_PROP_SOURCEID);
